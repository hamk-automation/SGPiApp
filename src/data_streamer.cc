...
#define ARDUINO_BAUD_RATE 2000000
...
// Combine the backed-up files
  ...
  RoadRecord* backup_recs = new RoadRecord[m_num_backups];
  ...
  // Save a new road record
  RoadRecord* rec = new RoadRecord();
  ...
  while (!m_flag_exit) {
    ...
    if (sick_daq_timer->TimeOut(SICK_ACQUISITION_CYCLE_TIME)) {
      ...
      if (r==0) { // Data received
        DEBUG_PRINT("Received new SICK data");
        ...
        g_data_pack->SetRotationAngle(g_rotation_angle);
...
void DataStreamer::AcquireArduinoData()
{
  ...
  while (!m_flag_exit) {
    ...
    if (mega_daq_timer.TimeOut(ARDUINO_ACQUISITION_CYCLE_TIME)) {
      RS232_SendByte(COMPORT_NUMBER, 'R');
      RS232_SendBuf(COMPORT_NUMBER, (unsigned char*)&g_velocity_setpoint, sizeof(float));
      RS232_flushTX(COMPORT_NUMBER);
      while (RS232_Available(COMPORT_NUMBER) < 21) { // Wait until receive the packet
      ...
      if (!(IsWeirdAngle(received_angle.yaw) ||
            IsWeirdAngle(received_angle.pitch) ||
            IsWeirdAngle(received_angle.roll)))
          {
            g_rotation_angle = received_angle;
          }
      g_velocity = *(float*)(&buf[12]);
      float t = *(float*)(&buf[16]) * (1000.0 / 30) * 180.0 * M_PI / (89.0 * 4);
      uint8_t last = *(uint8_t*)(&buf[20]); // Last byte

      if (t != g_velocity_setpoint) {
        ERROR_PRINT("Received different velocity setpoint response");
      }
      if (last != 0xFA) {
        ERROR_PRINT("Last byte is different (expected 0xFA)");
      }
      DEBUG_PRINT("velocity: %.2f, yaw: %.2f, pitch: %.2f, roll: %.2f, setpoint: %.2f",
        g_velocity,
        g_rotation_angle.yaw,
        g_rotation_angle.pitch,
        g_rotation_angle.roll,
        t
      );
      mega_daq_timer.Reset();
    }
...
// Start streaming data from SICK
  ...
  delete[] response;
  m_num_packs = 0;
  m_packs = new ScanDataPack[MAXNUMPACKS]; // Create packs vector
...
bool DataStreamer::ArduinoInit()
{
  ...
  // Connect to Arduino
    ...
    char buf[100];
    RS232_SendByte(COMPORT_NUMBER, 'R');
    RS232_SendBuf(COMPORT_NUMBER, (unsigned char*)&t, sizeof(float));
    RS232_flushTX(COMPORT_NUMBER);
    while (RS232_Available(COMPORT_NUMBER) < 21) {} // Wait until receive the packet
    RS232_PollComport(COMPORT_NUMBER, (unsigned char*)buf, 100);
    RS232_flushRX(COMPORT_NUMBER);
